...
Pattern * BytePattern::CreatePattern(const char *pattern) {
...
void BytePattern::DestroyPattern(Pattern *pattern) {
	delete pattern;
}

const void * BytePattern::Find(Pattern *pattern, const void *range_begin, size_t size) {
	if (!pattern) {
		return nullptr;
	}
